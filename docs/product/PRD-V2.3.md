# 产品需求文档：达芬奇密码 AI 素材工作台 - 内部产品化一阶段完整 PRD V2.4
...
V2.4 重点调整：首页新增「模板营地」，让非专业员工按想要的成片效果筛选图片或视频模板，上传/选择商品素材后直接创建任务；新建图片/视频任务内提供同一套模板入口。该能力复用既有模板版本、素材资产、五阶段门禁、Preflight、成本确认、异步生成和审核归档能力；不向用户暴露 Prompt、工作流节点、Dify 或 n8n。
...
- 不做联网真人图抓取、自动身份识别或无授权的人脸使用；用户声明授权后，可用上传人脸建立一个受审计的可复用模特档案。
- 不做 SKU 变体智能批量生成。
- 不做复杂 Excel 全字段导入。
- 不做无人确认的全自动批量生产。
- 不把飞书作为主数据源。
...
| 模特资源库 | 虚拟/授权模特档案库 | 设计/美工、管理员维护；运营可选择 | 支持文本、参考图和授权换脸三种候选图生成；不做联网抓取或自动身份识别 |
...
模特资源库用于创建、维护和选择可复用模特档案，不提供联网抓取或自动身份识别。用户可上传具有合法肖像及生成使用授权的人脸；系统必须记录来源、操作者、时间和声明版本。模特资源用于图片和视频任务中的模特风格推荐、人工选择和质量评估。
...
| 建档状态 | `draft`、`active`、`disabled`；只有已发布的 `active` 档案可被商品任务选择 |
| 脸部/形象锚点 | 分别保存有序脸部锚点与一张已选定的形象锚点 |
| 授权声明审计 | 上传人脸时保存源素材、操作者、时间、声明版本和来源说明 |
| 授权状态 | 已授权、待确认、不可用 |
...
| 商品资产 ID | 关联商品 |
| 媒体类型 | `image` 或 `video` |
| 任务 Profile | 用户可选商品 Profile 为 `main_image`、`scene_image`、`detail_image`、`tryon_three_view` 和视频 `img2video`、`reference2video`；内部模特资产 Profile 为 `model_face_anchor`、`model_identity_transfer` |
...
新建任务必须满足五阶段业务门禁。图片任务采用最短可见路径：来源素材与任务 Profile、自动上下文快照、最终 Prompt、通道参数与 Preflight、用户确认付费执行；自动上下文快照由服务端保存，浏览器不展示商品事实确认。视频仍保留来源事实冲突处理和人工确认。前端默认使用单页工作台承载这些阶段，不要求展示为独立页面、顶部 Stepper 或 AI 助手向导。Submit 后的排队、生成、审核和归档属于任务生命周期，不属于创建门禁。

```mermaid
flowchart TD
    A["选择主体素材（可选关联商品）"] --> B["AI 保存自动上下文快照"]
    B --> C["选择图片类型、参考图和标签"]
    C --> D["AI 助手生成或更新最终 Prompt"]
    D --> E["用户直接编辑最终 Prompt"]
    E --> G["选择模型通道"]
...
作为任务创建人，我希望选择主体素材后快速获得并编辑最终 Prompt，再选择模型和确认费用，以便用最少操作生成符合用途的图片。
...
3. 场景预置：室内、室外、动作/姿势。
4. 比例、分辨率和张数不使用全局固定选项，页面只展示当前模型通道实际支持的值；图片页采用能力默认值，比例和分辨率只在高级设置中修改，不提供独立的平台规格推荐入口。
5. 张数由用户设置，但受所选通道能力、后台上限和预算控制。
6. 普通员工使用推荐模板，可编辑任务级 Prompt 副本，不能修改全局模板。
7. AI 助手以服务端自动上下文快照、任务类型、风格、场景、姿势和有序参考图生成任务级 Prompt；浏览器不展示商品事实确认或工作流节点。主体素材必选，`product_asset_id` 和 SKU 可空。
8. 参考图上传后自动识别内容、角色建议及风格/场景/姿势标签，并刷新对应任务级 Prompt；用户编辑标签或 Prompt 只影响本次任务。
9. 图片页点击“生成”即隐式确认当前组 Prompt 并进入执行配置，不立即提交付费调用。模型选择后，页面根据能力 Schema 使用默认参数；高级设置才显示比例和分辨率。
...
- GIVEN 用户选择主体素材或上传参考图，WHEN AI 助手运行或参考图识别完成，THEN 系统生成或更新任务级 Prompt 副本和有效标签。
- GIVEN 普通员工编辑 Prompt，WHEN 保存，THEN 只影响本次任务。
- GIVEN 用户调整风格、场景、姿势或参考资产，WHEN 查看最终 Prompt，THEN 系统自动带入最新标签与参考图顺序并刷新任务级 Prompt 副本。
- GIVEN 用户缺少必填参数，WHEN 提交，THEN 系统提示缺失项。
- GIVEN 用户切换 Agnes 与其他生图通道，WHEN 通道能力不同，THEN 参数表单、可选比例、参考图限制和成本估算同步变化。
- GIVEN 用户点击生成，WHEN Prompt 快照已隐式确认但执行配置尚未确认，THEN 系统只能运行 Preflight，不得提交付费任务。
...
1. 新建 AI 模特只提供文本生成、参考图生成和换脸生成三种方式；页面固定为“输入 -> 生成候选 -> 保存”，不展示任务组、锚点或工作流节点。
2. 参考图生成从资源中心选择一张形象、姿势或风格参考图；换脸生成从资源中心分别选择脸部来源和目标形象两张图，保留目标图的身材、服装、姿势和场景。
3. 换脸生成必须在 Preflight 前勾选肖像及生成使用授权声明；系统记录源素材、操作者、时间、声明版本和来源说明。
4. 用户在当前模型能力范围内选择候选图数量（当前为 1-4 张）。点击生成候选图后，服务端仍执行 Preflight、用户确认费用和 Submit；页面返回对应数量的候选图。用户选定一张并保存后才发布为 `active`，未保存候选和失败结果不可被商品任务选择。
...
- GIVEN 用户在换脸模式未勾选授权声明，WHEN 点击生成候选图，THEN 系统阻止进入 Preflight。
- GIVEN 设计/美工选择候选图并保存，WHEN 从图片任务的模特参考入口创建，THEN 新档案自动回填当前任务；已手改 Prompt 仅展示 AI 新建议，不静默覆盖。
...
页面名称：新建任务  
所属菜单：生成任务 -> 新建任务  
页面用途：以主体素材快速生成最终 Prompt，并完成能力驱动的执行配置和付费生成确认。图片任务不展示商品事实确认或工作流节点；视频的来源冲突处理保持独立规则。

| 区域 | 元素与交互描述 |
| --- | --- |
| A. 页面导航与全局状态 | 保留三栏单页工作台。图片页不显示事实确认、保真规则、工作流节点、审核策略或平台规格推荐；素材、标签、Prompt、通道和参数在原区域就地编辑。上游变更使 Prompt、Preflight 和执行确认失效。 |
| B. 素材区 | 主体素材必选，关联商品和 SKU 可空；已关联时只读展示一行商品信息，未关联时正常生成并允许后续归档。左栏保留细节、风格、场景、姿势和模特五类参考图入口；其下展示可编辑的商品事实副本，包含名称、卖点、品类、颜色、图案/材质和版型/结构，并提供一键复制。编辑不是确认门禁，只影响本次任务 Prompt。 |
| C. 主要操作栏 | 中栏首屏展示多选图片类型、紧凑标签、AI 助手和所有已选类型纵向叠加的最终 Prompt。AI 助手只负责按素材、标签和图片类型生成或重生成 Prompt；上传参考图后自动识别并更新标签与 Prompt。未手动编辑的 Prompt 可自动更新；已手动编辑的 Prompt 必须显示变更来源和 AI 新建议，用户可查看差异、覆盖或保留。点击页面「生成」即隐式接受全部当前 Prompt，服务端先确认快照并运行 Preflight；只有弹窗中的「确认费用并生成」可以触发付费 Submit。 |
| D. 执行区 | 右栏只保留一个模型选择和各图片类型张数/总张数。比例与分辨率采用模型能力默认值，以折叠的高级设置呈现；后端过滤不可用模型，不在创建区展示成本、健康度、配额或独立的平台规格推荐。 |
| E. 最终确认 | Preflight 通过后，费用确认弹窗展示逐子任务参考图顺序、结构化参数、请求摘要、整组预计成本、耗时区间、队列健康度和兜底策略。任一子任务失败时整组不可提交。审核策略按组织或模板默认值写入执行快照，创建者不在页面选择。 |
...
```text
+--------------------------------------------------------------------------------+
| 新建图片任务  2 个类型 · 共 3 张                                      [生成] |
+----------------------+-----------------------------+---------------------------+
| 主体素材、参考图、事实  | 图片类型 / 标签 / 最终 Prompt | 模型与输出                |
| [选择主体素材]         | [主图][场景图][细节图][三视图]| 模型 [Agnes Image]         |
| 商品关联 [可选]        | 风格 / 场景 / 姿势 Chip       | 本次共 3 张                |
| 五类参考图入口         | [AI 助手：生成提示词]         | 高级设置 4:5 · 1536px      |
| 商品事实 [可编辑/复制] | Prompt [主图][场景图]…纵向叠加 |                             |
+----------------------+-----------------------------+---------------------------+
|                                                        [生成]                  |
+--------------------------------------------------------------------------------+
```
...
| D. 视频参数区 | 右栏选择支持 `img2video`、`reference2video` 或 `trending_replicate` Profile 的视频通道，并同时加载该通道对应 Prompt Profile 与动态参数。切换通道后基于已确认内容方案重新编译 Prompt、展示差异，再展示时长、比例、分辨率、运动幅度、Seed、参考图数量和顺序。仅语法变化保留阶段 3 确认；能力变化影响镜头、资产或语义时阻断并引导返回阶段 3。Preflight 展示预计成本/耗时、队列健康度、请求摘要和兜底策略。 |
...
1. 新建任务五阶段业务门禁：图片任务为来源素材与任务 Profile -> 自动上下文快照 -> 最终 Prompt -> 通道参数与 Preflight -> 用户确认付费执行；视频保留商品/来源事实确认。前端可在单页工作台内完成，不要求五屏向导。
...
- 用户可选图片 Profile 固定为 `main_image`、`scene_image`、`detail_image`、`tryon_three_view`；`model_face_anchor`、`model_identity_transfer` 仅用于模特资产建档，不显示在图片类型选择中。
- 视频 Profile 固定为 `img2video`、`reference2video`、`trending_replicate`；其中模板营地仅向普通用户展示「真人图参考」和「爆款复刻」两种业务类型。
...
## 18. V2.4 增补：首页模板营地与一键模板化

**需求位置**：本章是 V2.4 新增需求。用户入口位于「工作台首页 -> 模板营地」及「新建图片/视频任务 -> 模板」，不再新增独立二级页面；既有第 7.7 节「模板中心」继续承担专业配置与管理。它不改变第 17 章定义的工作流与平台能力边界。

### 18.1 第一性原理与版本目标

普通运营人员要解决的不是“如何组合 Prompt”，而是“我想要哪种可用成片，以及把我的商品放进去”。因此，首页与任务页的最短有效路径必须是：

```text
看到结果 -> 按用途/风格筛选 -> 选定模板 -> 选择商品素材 -> 少量必要补充 -> 确认费用并生成
```

页面面向不熟悉模型、参数和提示词的任务创建人。它把专业规则封装在结果模板内部，但不降低商品保真、成本确认、审核和资产归档的标准。

**本次目标**：

- 在首页建立「模板营地」，以图片卡和视频卡陈列成片效果；新建任务页提供同一套选择器。
- 让员工在不编辑 Prompt、不认识模型通道的情况下，完成一条合规的图片或视频生成任务。
- 将甲方提供的视觉方向沉淀为可版本化、可统计效果、可停用的结果模板，而非散落的参考图。
- 首发提供 10 个结果模板，验证模板点击率、从选模板到提交的转化率、首次出图通过率和单张通过成本。

**本次非目标**：

- 不在本页提供全量 Prompt 编辑、模型通道选择或工作流编排；专业用户仍使用「生成任务」和「模板中心」。
- 不将淘宝商品图、店铺标识、模特肖像、独创花型或详情页设计作为对外模板预览或生成参考图。
- 不做自动上架、自动批量生产、按店铺实时爬取、按店铺风格复刻或以 Dify/n8n 直接承载付费生成状态机。

### 18.2 已有资料与首发模板方案

研究素材位于 `/Users/jay/Desktop/02-projects/AIGC/原始资料/模版图片/`。其中 `淘宝商品套图/` 当前有 34 个按商品归档的「详情页五图」目录、193 张详情页图；`淘宝店铺采样/` 按店铺留存了 10-20 张图的风格样本。它们的用途仅为内部视觉研究和风格规律提炼，原始链接与采集时间保留在各商品目录的 `来源.tsv`。

研究结论不是照搬店铺画面，而是抽取用户能理解、可迁移且不依赖特定版权元素的视觉变量：明暗、色温、布光、镜头距离、构图密度、空间类型、模特姿态和画面节奏。

首发结果模板为 8 个图片模板和 2 个视频模板。每个结果模板至少提供 3 张甲方授权或系统自有生成的预览图；视频模板提供封面图和 3-6 秒循环预览。预览素材、内部规则和版本需要一起发布，不能只换一张封面图。

| 编号 | 结果模板 | 类型 | 结果感受与适用品类 | 来源样本的抽象方向 |
| --- | --- | --- | --- | --- |
| I01 | 静奢缎面晨光 | 图片 | 睡衣、家居服、轻奢女装；留白、柔和侧光、低饱和暖中性色 | 静奢深睡品质感、缎面材质和低噪声室内光线 |
| I02 | 东方雅致轻熟 | 图片 | 睡衣、连衣裙、轻熟女装；克制中式空间、端庄半身构图 | 东方雅致和轻熟人像的色彩、姿态与空间留白 |
| I03 | 新中式庭院出片 | 图片 | 新中式、改良旗袍、连衣裙；庭院、木质、自然光、纵向留白 | 新中式国风的场景材质和竖幅叙事，不复用具体纹样 |
| I04 | 甜酷暗黑辣妹 | 图片 | 吊带、短裙、潮流女装；低调深色背景、高对比边光、近景姿态 | 暗黑甜酷的色彩对比、镜头距离和情绪张力 |
| I05 | 多巴胺元气居家 | 图片 | 睡衣、居家服、少女款；明快纯色、抓拍感、轻松动作 | 多巴胺居家风的颜色密度、明亮布光和动态姿势 |
| I06 | 奶油甜妹卧室 | 图片 | 睡衣、家居服、少女款；奶油色空间、柔焦、亲近的日常感 | 店主风中卧室氛围、柔光与日常叙事 |
| I07 | 复古田园居家叙事 | 图片 | 棉麻睡衣、复古家居服；花园/窗边、胶片感、单品细节穿插 | 复古田园的色温、空间和故事性构图 |
| I08 | 高转化店主试穿 | 图片 | 睡衣、内搭、日常女装；真实试穿视角、商品清晰、版型可见 | 店主试穿的商品信息密度与可转化构图，不复制个人形象 |
| V01 | 静奢缎面慢镜头 | 视频 | 睡衣、缎面女装；轻微转身、抚平面料、稳定近中景 | I01 的柔光、材质特写和低运动幅度 |
| V02 | 多巴胺元气动态 | 视频 | 睡衣、少女家居服；轻快转身、坐卧/招手、明亮室内节奏 | I05 的色彩与轻快姿态，避免夸张变形动作 |

首发不以“店铺”命名模板。店铺是研究样本，不是可售卖的风格标签；对用户展示的是可解释的结果和使用场景。

### 18.3 技术方案评估与决策

| 方案 | 用户价值 | 新增成本 | 现有系统兼容性 | 可行性与风险 | 结论 |
| --- | --- | --- | --- | --- | --- |
| A. 首页模板营地 + 现有服务编排 | 高：选择结果后即可生成，保持可控确认 | 低到中：新增模板元数据、预览媒体、参数映射和首页/任务页入口 | 高：复用 `Content Registry`、任务组、Preflight、Submit、审核和资产库 | 高：调用链和审计归属保持不变 | **首发采用** |
| B. Dify 内置工作流 | 中：可快速搭建 Prompt/LLM 辅助 | 中到高：需要账号、知识库、版本、鉴权、Trace 和成本双重治理 | 中低：与现有任务组快照、通道能力、事务 Submit 存在双状态 | 不能承担素材资产、付费确认和异步生成的主状态机 | 不采用为主链路；以后可作为内部 Prompt 试验工具 |
| C. n8n 内置工作流 | 低：面向自动化管理员，不适合小白前台 | 中：节点运维、凭证、重试、幂等和故障定位 | 低：容易与现有 `Workflow Engine` 重复 | 外部回调或通知可用，付费生成主链路风险高 | 不采用为主链路；仅可用于非核心运营自动化 |
| D. 新建一套自研编排引擎 | 对首发无额外价值 | 高：重复建设状态机、队列、审计和适配器 | 低：与第 17 章能力重叠 | 建设长、测试面大 | 不采用 |

**决策理由**：付费生成的本质是一个需要可追溯、可预检、可幂等、可审核的业务交易，不是低代码流程画布。现有系统已明确拥有 `Workflow Engine`、`Content Registry`、`Generation Gateway` 和任务组提交能力。V2.4 只新增一个面向人的“选择器”和一个可版本化的“结果模板层”，把已经验证的底层链路重新包装为傻瓜式体验，价值/成本比最高。

```mermaid
flowchart TB
    U["任务创建人"] --> G["首页模板营地或新建任务模板选择器"]
    G --> S["按用途和风格筛选"]
    S --> P["选择模板及版本"]
    P --> A["选择商品素材"]
    A --> C["补充必要素材"]
...
一级菜单：模板中心
└── 二级页面：结果模板配置（V2.4 新增，仅设计/美工和管理员）
```

| 权限点 | 首页模板营地 / 任务页模板选择器 | 专业模板中心 | 说明 |
| --- | --- | --- | --- |
| `template:view` | 浏览模板和预览 | 只读（如授权） | 可见范围受工作区和数据范围控制 |
| `template:use` | 使用模板并写入当前任务草稿 | 无 | 不展示 Prompt、通道密钥和全局规则 |
| `template:publish` | 在已完成结果上「设为模板」 | 无 | 自动创建 V1；发布前校验结果快照完整性 |
| `template:edit` | 无 | 编辑模板并发布新版本 | 编辑不覆盖历史版本和已绑定任务 |
| `template:manage` | 无 | 下架、排序和查看效果 | 下架仅阻止后续使用，不影响历史查看 |

推荐授权组合：运营/任务创建人通常拥有 `template:view`、`template:use`；设计/美工通常额外拥有 `template:publish`、`template:edit`；管理员通常拥有全部权限。前端和接口均以权限点判断，不以角色名称硬编码。

新增核心对象只定义业务含义，不改变既有 `prompt_template`、任务组和素材资产的职责：

| 对象 | 必填核心字段 | 业务含义 |
| --- | --- | --- |
| `result_template` | `id`、名称、媒体类型、状态、品类/用途/风格/场景/姿势或镜头标签、封面资产、预览资产、来源任务/结果、使用/浏览/收藏聚合指标 | 用户在首页营地和任务页选择器中看到的完整结果方案 |
| `result_template_version` | 模板 ID、版本号、变更摘要、发布人、发布时间、不可变生成快照 | V1 从完成结果一键创建；后续编辑发布新版本，历史任务可回溯 |
| `result_template_usage` | 模板版本 ID、任务组 ID、用户、筛选条件快照、创建/提交时间 | 用于计算浏览、使用、提交、审核通过和成本的完整漏斗 |
| `result_template_favorite` | 模板版本 ID、用户、收藏时间 | 幂等记录当前用户收藏关系；前端只展示聚合收藏数，不以本地计数作为正式数据源 |

模板版本保存不可变的 Prompt、参考素材、模型、比例、分辨率、数量或视频时长/镜头结构快照。使用模板时，服务端将模板版本和素材角色写入当前任务草稿，再调用既有 `prompt-composer` 产出任务级 Prompt 副本。普通用户只看“将生成什么”，默认不编辑深度参数；主体商品素材始终可替换且为必填。

### 18.5 页面原型与交互规则

页面名称：首页工作台 / 模板营地；新建图片任务 / 新建视频任务的模板选择器
所属菜单：首页工作台；生成任务 -> 新建图片或视频任务
页面用途：让任务创建人通过成片结果选择模板，替换商品素材后生成图片或视频；不新增独立灵感页或第二套详情页。

| 区域 | 元素与交互描述 |
| --- | --- |
| A. 首页状态条 | 将渲染进度、数据和静态待办收敛为紧凑状态条，保留「新建图片」「新建视频」「进行中」「待处理」四个动作；待处理合并通知、审核、失败与补素材事项。 |
| B. 首页主 Tab | 默认「模板营地」，同级「我的作品」。模板营地不展示模型、Prompt 或工作流步骤；我的作品仅展示当前用户已完成图片/视频结果。 |
| C. 筛选与模板卡 | 按图片/视频、品类、用途、风格和排序筛选。保持全图卡视觉，桌面端悬停展开底部信息层，直接展示商品名称及“类型、风格、场景、姿势/镜头”的具体值；移除冗余“商品”字段名和详情按钮，点击预览区域进入详情。收藏为大尺寸心形按钮，未收藏描边、已收藏实心主色；触控端信息层默认可见，不依赖 hover。视频业务类型仅为「真人图参考」或「爆款复刻」，有预览时桌面端悬停循环播放；触控端保留封面与播放标识。右下角仅在来源商品与模板记录一致时展示原始白底商品图，禁止错配。 |
| C-1. 详情与互动 | 点击成片进入模板详情，展示商品、类型、标签、替换规则、收藏数和浏览数，并可直接「使用此模板」。用户可在卡片或详情收藏；浏览量在打开详情时记录。移动端详情改为预览在上、信息和操作在下的可滚动布局。 |
| D. 任务页模板选择器 | 新建图片/视频任务内以抽屉提供同一组模板和筛选。选择后写入当前草稿，不创建付费任务；图片带入 Prompt、参考素材、模型、比例、分辨率和数量，视频额外锁定时长和镜头结构。 |
| E. 素材替换与确认 | 用户在既有任务页选择已入库商品图或上传商品图。主体素材必填；视频还必须提供合格首帧/主体图。随后沿用既有 Preflight、费用确认和 Submit。 |
| F. 我的作品与既有详情 | 已完成结果提供「查看详情」「再次创作」「设为模板」。查看详情跳转任务列表并打开既有详情抽屉；设为模板直接创建 V1，不新增发布表单或详情页。发布后按钮变为「下架模板」；下架后变为「重新上架」。下架需 `template:manage` 权限，仅阻止新任务使用，不影响历史任务和版本回溯。 |

ASCII 原型（已确认实施版）：

```text
+------------------------------------------------------------------------------------------------+
| 创作工作台                                  [新建图片] [新建视频] [进行中] [待处理]          |
+------------------------------------------------------------------------------------------------+
| [模板营地 10] [我的作品 2]    媒介 [全部 v]  品类 [全部 v]  用途 [全部 v]  风格 [全部 v]      |
+------------------------------------------------------------------------------------------------+
| 模板营地                                                                                         |
| +----------------------+ +----------------------+ +----------------------+ +------------------+ |
| | [预览图 4:5]         | | [预览图 4:5]         | | [预览图 4:5]         | | [视频预览 9:16] | |
| | 静奢缎面晨光          | | 东方雅致轻熟          | | 甜酷暗黑辣妹          | | 静奢慢镜头      | |
| | 柔光、质感、可上架    | | 克制、雅致、留白      | | 深色、利落、有张力    | | 8 秒 · 轻微运镜 | |
| | 适合：睡衣/轻奢女装   | | 适合：轻熟女装        | | 适合：潮流女装        | | 适合：缎面睡衣  | |
| |              [使用]  | |              [使用]  | |              [使用]  | |          [使用] | |
| +----------------------+ +----------------------+ +----------------------+ +------------------+ |
|                                                                                                  |
| 我的作品 Tab： [作品卡] [查看详情] [再次创作] [设为模板]                                        |
| +----------------------+ +----------------------+ +----------------------+ +------------------+ |
| | 新中式庭院出片        | | 多巴胺元气居家        | | 奶油甜妹卧室          | | 高转化店主试穿  | |
| +----------------------+ +----------------------+ +----------------------+ +------------------+ |
+------------------------------------------------------------------------------------------------+
...
+------------------------------------------+-----------------------------------------------------+
| 模板快照（只读）                           | 用你的商品替换素材                                  |
| 4 张 · 4:5 · 柔和侧光 · 低饱和            | 主体商品图 *                                        |
| 参考画面和深度参数已自动带入               | [从素材库选择] [上传图片]                            |
|                                           | 已选：真丝家居服白底图.jpg                           |
|                                           |                                                     |
|                                           | [进入 Preflight] [确认费用并生成]                    |
+------------------------------------------+-----------------------------------------------------+
```

关键业务规则：

1. 选择模板卡不会立即创建付费任务；只有完成素材选择、预览费用并确认后才创建或提交任务组。
2. 用户可跳过建议素材，但不能跳过模板规定的主体素材。主体素材不符合最小尺寸、格式、主体可见性时，禁止进入 Preflight。
3. 图片模板默认输出图片。视频模板的用户侧业务类型固定为两种：`真人图参考` 使用 `reference2video`，必须补齐商品首帧并带入模板固化的真人/场景参考；`爆款复刻` 使用 `trending_replicate`，必须使用内部授权源视频、补齐商品替换图并完成结构分析。两类模板均锁定其时长与镜头节奏，普通用户不需要编辑底层 Profile。
4. 模板版本、来源任务/结果、生成快照、素材角色与用户填写的受控变量都必须写入任务快照和操作日志。
5. 已停用模板的历史任务可查看，不能再次使用；版本编辑产生新版本，不能覆盖历史任务绑定的版本。
6. 缺少 Prompt、主体替换位或必要输出参数的完成结果，必须禁用「设为模板」并说明原因。

### 18.6 用户故事与验收标准

#### US-18-01：按成片效果选择模板

作为运营，我希望先看成片效果再选模板，以便不理解 Prompt 也能找到适合商品的出图方向。

验收标准：

- GIVEN 用户进入首页，WHEN 未使用筛选，THEN 系统默认展示「模板营地」和图片/视频模板数量。
- GIVEN 用户选择“睡衣/家居服”和“静奢”，WHEN 筛选完成，THEN 仅显示匹配的已发布模板，卡片不显示 Prompt、模型通道或参考店铺。
- GIVEN 用户浏览模板卡，WHEN 在桌面端悬停或使用触控设备，THEN 商品名称、类型及风格/场景/姿势或镜头标签的具体值可见；卡片文字不得溢出或遮挡操作。
- GIVEN 视频模板有预览，WHEN 用户浏览模板卡或在任务页选择器中选择模板，THEN 系统展示封面、时长和可理解的视频标识；播放失败时展示封面图及提示。
- GIVEN 用户点击模板成片，WHEN 详情打开，THEN 展示浏览量并记录一次浏览；WHEN 用户切换收藏状态，THEN 当前用户收藏关系与聚合收藏数同步更新，且不会触发创建任务。

#### US-18-02：替换商品素材并受控生成

作为运营，我希望只替换我的商品素材，就能得到接近模板预览的成片，以便快速开始生成。

验收标准：

- GIVEN 用户点击图片模板的“使用此模板”，WHEN 选择合格主体商品图，THEN 系统显示模板用途、默认输出和可选补充素材，不要求编辑 Prompt。
- GIVEN 用户点击视频模板的“使用此模板”，WHEN 未提供合格首帧，THEN 系统阻止继续并说明需要上传或选择主体图。
- GIVEN 用户完成素材选择，WHEN 点击生成，THEN 系统将模板版本、来源结果、生成快照和素材角色写入草稿并调用既有 Preflight。
- GIVEN Preflight 通过，WHEN 用户在费用确认弹窗点击“确认费用并生成”，THEN 系统以既有事务 Submit 创建任务组；不出现第二套任务状态或队列。

#### US-18-03：从完成结果一键设为模板

作为拥有发布权限的用户，我希望把验证过的完成结果直接设为模板，以便持续提升小白用户的首次出图成功率。

验收标准：

- GIVEN 用户拥有 `template:publish`，WHEN 已完成结果的 Prompt、主体替换位和必要生成参数完整，THEN 点击「设为模板」后系统直接创建 V1 并出现在首页模板营地。
- GIVEN 用户缺少 `template:publish` 或结果快照不完整，WHEN 查看该结果，THEN 操作禁用并显示具体原因。
- GIVEN 模板发生编辑，WHEN 发布新版本，THEN 历史任务仍可回溯旧版本和旧规则。
- GIVEN 用户拥有 `template:manage` 并下架模板，WHEN 用户访问历史任务，THEN 历史结果正常可看；WHEN 用户新建任务，THEN 该模板不可选。
- GIVEN 用户在我的作品或任务详情中已发布模板，WHEN 点击「下架模板」，THEN 模板状态变为已下架且按钮变为「重新上架」；WHEN 点击「重新上架」，THEN 模板恢复可用于新任务。

#### US-18-04：待处理与任务详情复用

作为任务创建人，我希望在一个待处理入口看到需要行动的事项，并直接查看关联任务，以便不遗漏审核、失败和补素材工作。

验收标准：

- GIVEN 顶栏或首页状态条显示待处理数，WHEN 用户打开待处理中心，THEN 审核、失败、补素材与普通系统消息仅在同一个入口展示，并支持已读状态。
- GIVEN 用户点击任务类待处理事项或我的作品的「查看详情」，WHEN 目标是图片或视频任务，THEN 系统切换到正确媒介和批次，并打开既有任务详情抽屉。

### 18.7 数据指标、实施计划与风险

首发模板的去留以 2-4 周的真实使用数据判断，而不是仅凭主观审美。核心指标：模板卡点击率、模板使用到素材选择转化率、素材选择到付费确认转化率、首次生成通过率、平均审美分、单张通过成本、模板被打回的首要问题标签。模板效果看板需按模板版本、商品品类、创建人和模型通道拆分。

| 阶段 | 范围 | 交付与验收 |
| --- | --- | --- |
| P0 模板资产定稿 | 10 个结果模板、每个模板的授权/自有预览、内部视觉规则、素材要求、负面约束和关联生成快照 | 设计/业务确认首发模板清单；没有授权预览的模板不得发布 |
| P1 数据与后台 | 模板/版本/使用记录，专业模板中心配置入口，RBAC 与审计 | 可一键发布、停用、版本回溯；能从一次任务找到完整模板快照 |
| P2 用户页面 | 首页模板营地、我的作品、图片/视频任务内模板选择器、素材替换和详情跳转 | 小白用户在不见 Prompt 的条件下完成一条图片和一条视频任务 |
| P3 效果闭环 | 漏斗事件、模板效果看板、低效模板下架与灰度 | 可按模板版本判断保留、优化或停用，不以单张结果做结论 |

| 风险 | 影响 | 应对 |
| --- | --- | --- |
| 用竞品原图作为预览或参考 | 侵权、品牌混淆、输出趋同 | 采样图仅内部研究；预览必须是甲方授权或系统自有生成素材；不保留店铺名为用户标签 |
| 模板预览好看但替换商品后失真 | 用户信任下降、审核失败 | 为每个包定义素材最低要求、适用品类和禁止品类；先以少量品类灰度，依据通过率优化 |
| 模板太多导致选择困难 | 转化率下降 | 首发限制 10 包，按用途优先而不是堆砌风格；推荐位基于品类和近期使用情况 |
| 与专业模板中心规则双写 | 历史不可追溯、维护成本上升 | 首页仅承担发现、使用与一键发布；配置仍统一在专业模板中心，使用版本快照回溯 |
| 引入 Dify/n8n 形成两套状态机 | 费用、重试和审计失控 | 付费生成只走现有 Workflow Engine；低代码工具仅限未来的非核心运营自动化 |

### 18.8 API 与实施影响说明

本章明确产品行为，正式接口字段和调用顺序需要在 `docs/product/API-CONTRACT-V2.3.md` 的下一迭代同步补充。至少需要：模板列表/详情、从完成结果发布模板、版本编辑/下架、预览媒体签名访问、模板使用记录、待处理事项和模板效果统计。现有 `/v1/prompt-templates` 仍只服务专业模板中心，不能直接承担用户侧模板营地的全部展示语义。

在前端 SPA 中，首页 `Dashboard` 承担模板营地和我的作品；图片/视频新建任务共用模板选择器；任务详情继续复用 `TaskList` 和 `TaskDetailsDrawer`。保持 `TemplateCenter.tsx` 为专业配置页。路由仍沿用当前 `App.tsx` 的 `currentScreen` 状态，不引入 React Router、状态管理库或新的前端依赖。

本次不要求修改数据库 schema、API 契约或生产配置；开发实施前由后端子流程将上述对象映射为正式 DTO 和持久化结构。

## 19. V2.4 PRD 总集登记行

```markdown
| PRD-002 | 达芬奇密码 AI 素材工作台 - V2.4 首页模板营地 | 首页以模板营地和我的作品为主要内容，任务页提供同一模板入口；设计师可从已完成图片或视频一键设为模板。普通用户替换主体素材后复用既有 Preflight、费用确认、异步生成和归档链路。RBAC 控制模板操作，不以角色硬编码；不以 Dify 或 n8n 承担付费生成主链路。淘宝样本仅用于内部 mock 与研究，正式预览使用授权或自有素材。 | docs/product/PRD-V2.3.md#18-v24-增补首页模板营地与一键模板化 |
```
