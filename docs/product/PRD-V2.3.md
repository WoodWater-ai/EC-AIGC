...
V2.4 重点调整：新增独立二级页面「灵感模板」，让非专业员工按想要的成片效果筛选图片或视频模板，上传/选择商品素材后直接创建任务。该页复用既有模板版本、素材资产、五阶段门禁、Preflight、成本确认、异步生成和审核归档能力；不向用户暴露 Prompt、工作流节点、Dify 或 n8n。
...
**需求位置**：本章是 V2.4 新增需求，位于既有第 7.7 节「模板中心」之后的独立业务入口。它不替换第 7.7 节的专业模板维护能力，也不改变第 17 章定义的工作流与平台能力边界。

### 18.1 第一性原理与版本目标

普通运营人员要解决的不是“如何组合 Prompt”，而是“我想要哪种可用成片，以及把我的商品放进去”。因此，本页的最短有效路径必须是：

```text
看到结果 -> 按用途/风格筛选 -> 选定模板 -> 选择商品素材 -> 少量必要补充 -> 确认费用并生成
```

页面面向不熟悉模型、参数和提示词的任务创建人。它把专业规则封装在模板包内部，但不降低商品保真、成本确认、审核和资产归档的标准。

**本次目标**：

- 建立「灵感模板」独立二级页面，以图片卡和视频卡陈列成片效果。
- 让员工在不编辑 Prompt、不认识模型通道的情况下，完成一条合规的图片或视频生成任务。
- 将甲方提供的视觉方向沉淀为可版本化、可统计效果、可停用的模板包，而非散落的参考图。
- 首发提供 10 个模板包，验证模板点击率、从选模板到提交的转化率、首次出图通过率和单张通过成本。
...
首发模板包为 8 个图片模板和 2 个视频模板。每个模板包至少提供 3 张甲方授权或系统自有生成的预览图；视频模板提供封面图和 3-6 秒循环预览。预览素材、内部规则和版本需要一起发布，不能只换一张封面图。

| 编号 | 模板包 | 类型 | 结果感受与适用品类 | 来源样本的抽象方向 |
| --- | --- | --- | --- | --- |
...
| --- | --- | --- | --- | --- | --- |
| A. 模板包页面 + 现有服务编排 | 高：选择结果后即可生成，保持可控确认 | 低到中：新增模板包元数据、预览媒体、参数映射和页面 | 高：复用 `Content Registry`、任务组、Preflight、Submit、审核和资产库 | 高：调用链和审计归属保持不变 | **首发采用** |
...
**决策理由**：付费生成的本质是一个需要可追溯、可预检、可幂等、可审核的业务交易，不是低代码流程画布。现有系统已明确拥有 `Workflow Engine`、`Content Registry`、`Generation Gateway` 和任务组提交能力。V2.4 只新增一个面向人的“选择器”和一个面向配置的“模板包层”，把已经验证的底层链路重新包装为傻瓜式体验，价值/成本比最高。
...
一级菜单：模板中心
└── 二级页面：模板包配置（V2.4 新增，仅设计/美工和管理员）
```

| 角色 | 灵感模板页 | 模板包配置 | 说明 |
| --- | --- | --- | --- |
| 运营/任务创建人 | 浏览、筛选、使用、保存草稿、查看自己的结果 | 无 | 不能查看或编辑底层 Prompt、通道密钥和全局规则 |
| 设计/美工 | 同运营，并可查看模板质量标签 | 新建、编辑、发布版本、停用、查看效果 | 发布前需补齐预览、可用素材条件和关联模板版本 |
| 审核人 | 浏览、创建（如已有创建权限） | 只读 | 审核标准沿用既有规则 |
| 管理员 | 全部 | 全部，含排序、推荐位、权限和停用 | 可配置首发模板及灰度范围 |

新增核心对象只定义业务含义，不改变既有 `prompt_template`、任务组和素材资产的职责：

| 对象 | 必填核心字段 | 业务含义 |
| --- | --- | --- |
| `result_template_package` | `id`、名称、类型、状态、排序、适用品类、用途标签、风格标签、封面资产、预览资产、默认输出数 | 用户在灵感模板页看到和选择的完整结果方案 |
| `result_template_variant` | `id`、package_id、名称、预览资产、默认场景/动作、受控变量定义、关联图片/视频任务模板版本、默认通道策略 | 同一个结果方向的可选变体，例如“窗边全身”和“床边半身” |
| `result_template_publish_version` | 包版本、变更摘要、发布人、发布时间、状态 | 确保历史任务能回溯当时的结果模板、预览和内部规则 |
| `result_template_usage` | package_version_id、variant_id、task_group_id、用户、筛选条件快照、创建/提交时间 | 用于计算从浏览到提交、审核通过和成本的完整漏斗 |

模板包只能引用“已发布”的底层模板版本，不能内嵌可变 Prompt 正文。创建任务时，服务端写入模板包版本和变体快照，再调用既有 `prompt-composer` 产出任务级 Prompt 副本。用户在灵感模板页只看“将生成什么”，不看 Prompt；如确需深度调整，可显式跳转到现有专业新建任务页，跳转后才允许编辑。

### 18.5 页面原型与交互规则

页面名称：灵感模板
所属菜单：生成任务 -> 灵感模板
页面用途：让任务创建人通过成片结果选择模板，替换商品素材后生成图片或视频。

| 区域 | 元素与交互描述 |
| --- | --- |
| A. 页面导航与全局状态 | 顶部显示「灵感模板」、图片/视频分段切换、我的草稿和我的任务入口；不展示模型、Prompt 或工作流步骤。 |
| B. 筛选与查询区 | 以自然语言标签筛选：用途、品类、风格、氛围、画面类型；支持关键词搜索。首屏默认展示推荐模板和全部标签，不要求用户先填表。 |
| C. 核心内容展示区 | 固定比例的图片/视频卡片网格。图片卡显示 3 张预览的悬停切换；视频卡展示静音循环预览、时长和首帧。卡片只显示模板名、结果描述、适用商品、图片/视频标识与「使用此模板」。 |
| D. 模板详情抽屉 | 左侧预览轮播或视频播放器，右侧显示适用商品、会产出什么、示例构图、默认张数、必要素材说明和「使用此模板」。不显示原始参考店铺、Prompt、模型参数。 |
| E. 素材替换工作区 | 模板确定后在页内抽屉/独立工作区选择已入库商品图或上传商品图。必填项仅为主体素材；模板有额外要求时显示具体、可理解的补充项，例如“建议补充一张面料细节图”。 |
| F. 确认与异常状态 | 确认页显示模板结果预览、已选素材、输出数量、预计费用和预计完成时间。用户点击确认后才进入既有 Preflight/Submit。素材不符合要求时说明原因并提供替换入口；模板停用、预览加载失败、无匹配模板和权限不足均有明确空/错状态。 |
...
| 生成任务 / 灵感模板                                      [我的草稿] [我的任务]                  |
+------------------------------------------------------------------------------------------------+
| [图片 8] [视频 2]    用途 [全部 v]  品类 [睡衣/家居服 v]  风格 [全部 v]  [搜索模板]          |
+------------------------------------------------------------------------------------------------+
| 推荐给你                                                                                       |
| +----------------------+ +----------------------+ +----------------------+ +------------------+ |
| | [预览图 4:5]         | | [预览图 4:5]         | | [预览图 4:5]         | | [视频预览 9:16] | |
| | 静奢缎面晨光          | | 东方雅致轻熟          | | 甜酷暗黑辣妹          | | 静奢慢镜头      | |
| | 柔光、质感、可上架    | | 克制、雅致、留白      | | 深色、利落、有张力    | | 6 秒 · 轻运动   | |
...
+------------------------------------------+-----------------------------------------------------+
| 模板会生成什么                            | 用你的商品替换素材                                  |
| [预览 1] [预览 2] [预览 3]                | 主体商品图 *                                        |
| 柔和侧光、低饱和、材质清晰的室内成片       | [从素材库选择]  [上传图片]                           |
| 默认生成：4 张 4:5 商品场景图              | 已选：真丝家居服白底图.jpg                           |
|                                          |                                                     |
| 适合：睡衣、家居服、轻奢女装              | 可选补充                                             |
| 建议：补一张面料细节图，能提升材质表现      | [上传面料细节图]                                     |
|                                          |                                                     |
|                                          | [取消]                         [查看并生成]          |
+------------------------------------------+-----------------------------------------------------+
```
...
3. 图片模板默认输出图片；视频模板必须要求一个合格首帧/主体图，并复用既有 `img2video` 或 `reference2video` Profile。视频首发只提供轻运动，不开放镜头编辑。
4. 模板包版本、变体、底层模板版本、素材角色与用户填写的受控变量都必须写入任务快照和操作日志。
5. 用户返回模板列表后保留已选筛选条件和草稿；已停用模板的历史任务可查看，不能再次使用。
6. 用户点击“深度调整”才跳转专业新建任务页；跳转会保留模板包快照和素材，但由用户自行承担后续 Prompt 编辑结果。
...
- GIVEN 用户进入灵感模板页，WHEN 未使用筛选，THEN 系统在首屏显示推荐模板和图片/视频数量。
- GIVEN 用户选择“睡衣/家居服”和“静奢”，WHEN 筛选完成，THEN 仅显示匹配的已发布模板包，卡片不显示 Prompt、模型通道或参考店铺。
- GIVEN 视频模板有预览，WHEN 用户悬停或打开详情，THEN 系统展示静音循环视频和时长；播放失败时展示封面图及可理解提示。
...
- GIVEN 用户完成素材选择，WHEN 点击“查看并生成”，THEN 系统将模板包版本、变体、底层模板版本和素材角色写入草稿并调用既有 Preflight。
- GIVEN Preflight 通过，WHEN 用户在费用确认弹窗点击“确认费用并生成”，THEN 系统以既有事务 Submit 创建任务组；不出现第二套任务状态或队列。

#### US-18-03：设计/美工维护模板包

作为设计/美工，我希望把验证过的结果做成模板包并查看效果，以便持续提升小白用户的首次出图成功率。

验收标准：

- GIVEN 设计/美工新建模板包，WHEN 缺少封面、3 张图片预览或关联的已发布底层模板版本，THEN 系统不允许发布。
- GIVEN 模板包被发布，WHEN 普通员工刷新灵感模板页，THEN 按配置的排序、权限和灰度范围可见。
- GIVEN 已发布模板包的受控变量或关联底层模板版本发生改变，WHEN 发布新版本，THEN 历史任务仍可回溯旧包版本和旧规则。
- GIVEN 管理员停用模板包，WHEN 用户访问历史任务，THEN 历史结果正常可看；WHEN 用户从模板页创建新任务，THEN 该模板不可选。

### 18.7 数据指标、实施计划与风险

首发模板的去留以 2-4 周的真实使用数据判断，而不是仅凭主观审美。核心指标：模板卡点击率、模板详情到素材选择转化率、素材选择到付费确认转化率、首次生成通过率、平均审美分、单张通过成本、模板被打回的首要问题标签。模板包效果看板需按模板版本、变体、商品品类、创建人和模型通道拆分。

| 阶段 | 范围 | 交付与验收 |
| --- | --- | --- |
| P0 模板资产定稿 | 10 个模板包、每包授权/自有预览、内部视觉规则、素材要求、负面约束和关联底层模板版本 | 设计/业务确认首发模板清单；没有授权预览的模板不得发布 |
| P1 数据与后台 | 模板包/变体/版本/使用记录，模板中心配置入口，RBAC 与审计 | 可发布、停用、版本回溯；能从一次任务找到完整模板包快照 |
| P2 用户页面 | 灵感模板筛选、图片/视频预览、详情、素材替换、草稿与创建跳转 | 小白用户在不见 Prompt 的条件下完成一条图片和一条视频任务 |
| P3 效果闭环 | 漏斗事件、模板效果看板、低效模板下架和变体 A/B 灰度 | 可按模板版本判断保留、优化或停用，不以单张结果做结论 |
...
| 与专业模板中心规则双写 | 历史不可追溯、维护成本上升 | 模板包仅引用底层模板发布版本，不能复制 Prompt；统一走 Content Registry 与版本快照 |
| 引入 Dify/n8n 形成两套状态机 | 费用、重试和审计失控 | 付费生成只走现有 Workflow Engine；低代码工具仅限未来的非核心运营自动化 |

### 18.8 API 与实施影响说明

本章明确产品行为，正式接口字段和调用顺序需要在 `docs/product/API-CONTRACT-V2.3.md` 的下一迭代同步补充。至少需要：模板包列表/详情、后台 CRUD 与发布、预览媒体签名访问、从模板包创建任务草稿、模板包使用事件和模板效果统计。现有 `/v1/prompt-templates` 仍只服务专业模板中心，不能直接承担用户侧模板包的全部展示语义。

在前端 SPA 中，建议新增 `AppScreen.INSPIRATION_TEMPLATES` 和独立组件 `src/components/InspirationTemplateGallery.tsx`；保持 `TemplateCenter.tsx` 为专业配置页。路由仍沿用当前 `App.tsx` 的 `currentScreen` 状态，不引入 React Router、状态管理库或新的前端依赖。
...
```markdown
| PRD-002 | 达芬奇密码 AI 素材工作台 - V2.4 灵感模板结果页 | 新增生成任务下的独立二级页面灵感模板。普通员工按成片效果筛选 8 个图片模板和 2 个视频模板，替换商品素材后复用现有模板版本、五阶段门禁、费用确认、异步生成、审核和商品资产归档。首发不暴露 Prompt、模型或工作流，不以 Dify 或 n8n 承担付费生成主链路。淘宝样本仅用于内部风格研究，产品预览须使用甲方授权或系统自有素材。 | docs/product/PRD-V2.3.md#18-v24-增补灵感模板结果页 |
```
