...
V2.3 重点调整：在 V2.2 业务规则基础上合并工作流与 Skills 蓝图，正式收口任务组与子任务模型、结果血缘、五步创建流程、组级 Preflight/Submit、五个 AI Skills 和七个平台能力；任何付费生成必须经过“确认 Prompt -> 选择通道和参数 -> Preflight -> 用户确认执行 -> Submit”。
...
| 模型通道 | 用户从当前任务可用通道中选择；系统可以推荐，但不得在用户确认后静默切换 |
| 通道参数 | 根据所选通道能力动态生成，包括比例、分辨率、张数、参考图数量、时长、运动幅度等结构化参数 |
| 全局模板版本 | 创建任务时引用的模板版本 |
| 任务级 Prompt | 由全局模板、已确认商品事实、任务目标、拍摄画面、风格、场景、参考资产、自由描述和负面约束形成，可编辑本次副本；比例、分辨率、张数、时长和模型通道属于执行快照，不得只依赖 Prompt 文本表达 |
...
| --- | --- |
| 执行快照 ID | 系统唯一标识 |
| 任务 ID / Prompt 快照 ID | 关联任务和已确认 Prompt |
...
新建任务固定为五步：来源素材与任务 Profile、商品/来源事实确认、Prompt 内容确认、通道参数与 Preflight、用户确认付费执行。Submit 后的排队、生成、审核和归档属于任务生命周期，不增加向导步骤。
...
| 区域 | 元素与交互描述 |
| --- | --- |
| A. 页面导航与全局状态 | 顶部五步条：来源素材与任务 Profile -> 商品/来源事实确认 -> Prompt 内容确认 -> 通道参数与 Preflight -> 确认付费执行。每一步保存独立快照；修改上游内容后，下游确认自动失效。 |
| B. 筛选与查询区 | 可从已有商品资产搜索商品名，也可直接新建商品资产；新建时以当前上传/选中的商品主体图初始化任务。 |
| C. 主要操作栏 | 素材阶段提供「目录扫描」「批量上传」「保存草稿」；内容阶段提供「确认商品」「确认 Prompt」；执行阶段提供「运行预检」「确认并生成」。任何付费调用只能由最后一个按钮触发。上下装合成作为左侧素材上传区内的快速功能。 |
...
| E. 详情/编辑面板 | 内容确认区展示商品事实快照、各子任务 Prompt、变量来源、自由描述、负面约束和推荐模特；执行确认区展示逐子任务参考图顺序、结构化参数、请求摘要和校验结果，以及整组预计总成本、耗时区间、队列健康度和兜底策略。任一子任务失败时整组不可提交。用户修改 Prompt、参考图、通道或任一执行参数后，旧执行确认立即失效并要求重新预检。 |
...
```text
+--------------------------------------------------------------------------------+
| 新建任务  来源素材与 Profile > 确认事实 > 确认 Prompt > 参数与 Preflight > 付费确认 |
+----------------------+-----------------------------+---------------------------+
| 素材与参考图         | 内容确认                    | 执行确认                  |
...
| 区域 | 元素与交互描述 |
| --- | --- |
| A. 页面导航与全局状态 | 顶部使用与图片一致的五步条，并显示任务来源、商品名、视频任务 ID、自动保存状态；右上角固定“AI 助手”入口。 |
| B. 来源图片区 | 左栏选择来源图片：已生成图片、已通过图片、候选图片、商品素材包；也允许上传参考图或首帧图。每张图必须具有角色和从 1 开始的唯一顺序，可调整顺序。若从图片任务列表点击“创建视频”进入，自动选中来源图片并带入商品信息。 |
| C. 视频 Prompt 区 | 中栏展示视频 Prompt 模板推荐、备选模板、模板版本、结构建议和任务级视频 Prompt 副本。系统根据来源图片的视觉理解结果、商品信息、图片角色、视频模式和目标时长生成内容建议；推荐模板只是初始建议，用户可手动切换。三段式结构只能作为系统建议，不可写死。用户确认 Prompt 后进入执行配置，不自动提交视频任务。 |
| D. 视频参数区 | 右栏先选择支持 `img2video` 或 `reference2video` Profile 的视频通道，再根据有序能力字段展示模式、时长、比例、分辨率、运动幅度、Seed、参考图数量和顺序。Preflight 展示预计成本/耗时、队列健康度、请求摘要和兜底策略；修改任一内容后需重新确认执行。 |
| E. AI 助手边栏 | 从右上角打开，读取商品信息、来源图片、图片理解结果、推荐模板、视频时长、比例、模型通道、当前三段式建议和任务级视频 Prompt，输出模板推荐解释、分段建议、Prompt 改写建议和风险提醒；应用建议必须由用户确认，只影响本次视频任务副本。 |
| F. 底部操作栏 | 保存草稿、取消、确认视频 Prompt、运行预检、确认并生成。只有“确认并生成”会创建付费 Provider 任务；成功提交后进入异步视频队列。 |
...
| 审核分/通过状态       | 段1 [开场主体/建议时段]       | 比例 [9:16]          |
| 商品信息摘要         | 段2 [动态展示/建议时段]       | 分辨率 [1080p]       |
|                      | 段3 [细节收束/建议时段]       | 运动幅度 [small]     |
|                      | [任务级视频 Prompt 副本]      | 通道 [Vidu Q3]       |
|                      | [确认视频 Prompt]             | 请求预览/成本/健康度 |
+----------------------+----------------------------+----------------------+
| [保存草稿]                            [运行预检] [确认并生成]            |
+--------------------------------------------------------------------------+
```
...
| 视频 Prompt 模板 | 视频模式、适用图片类型、适用品类、推荐条件、默认时长、默认比例、默认分辨率、默认运动幅度、默认模型通道、三段结构建议、视频 Prompt 正文、变量字段、引用负面约束 |
...
- 视频 Prompt 模板用于定义“图片如何动起来”，需支持根据图片理解结果、商品信息、图片类型、视频模式、时长、比例和模型通道进行模板推荐。
...
| 视频 Prompt 模板 | 这张商品图应该怎么动起来？ | 管“怎么动起来”，提供视频模板推荐、三段式结构建议和视频 Prompt |
...
1. 新建任务五步流程：来源素材与任务 Profile -> 商品/来源事实确认 -> Prompt 内容确认 -> 通道参数与 Preflight -> 用户确认付费执行。
...
- 一次五步创建流程对应一个 `generation_task_group`；多图片 Profile 形成多个子任务，单视频同样使用只有一个子任务的任务组。
