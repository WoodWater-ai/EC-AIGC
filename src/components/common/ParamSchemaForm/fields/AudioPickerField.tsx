...
import { AudioLines, X, Upload } from 'lucide-react';
...
export default function AudioPickerField({ field, value, onChange }: AudioPickerFieldProps) {
      ...
      {value ? (
        <div className="flex items-center gap-3 p-3 border border-slate-200 rounded-lg bg-slate-50">
          <div className="w-16 h-16 rounded bg-slate-200 flex items-center justify-center shrink-0">
            <AudioLines className="w-6 h-6 text-slate-500" />
          </div>
        ...
        >
          ...
          <span className="text-sm text-slate-600">选择音频 (mp3/wav/aac/m4a)</span>
        </button>
      )}
