...
/**
 * Header —— 2026-07-09 接入 AdminNotificationController
 *
 * 改造点：
 * 1. 移除 props 里的 notifications / markAllAsRead（App.tsx 不再传）
 * 2. 内部管理 notifications state
 * 3. 进入页面 + 每 60s 拉 unreadCount
 * 4. 下拉打开时拉全量列表
 * 5. 点击单条调 mark-read
 * 6. "全部已读" 调 mark-all-read
 */
export const Header: React.FC<HeaderProps> = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const page = await notificationApi.myPage({ pageNum: 1, pageSize: 20 });
        if (active) setNotifications(page.list ?? []);
      } catch {
        // axios 已 toast
      }
    })();
  ...
  const getBreadcrumb = () => {
    switch (currentScreen) {
      ...
      case AppScreen.SYSTEM_ROLE_MGMT:
        return { parent: '运维与渠道', child: '角色管理' };
      case AppScreen.SYSTEM_MENU_MGMT:
        return { parent: '运维与渠道', child: '菜单管理' };
      default:
  ...
  return (
    <header className="h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between select-none relative z-40">
      ...
      <div className="flex items-center gap-2 text-xs text-slate-400">
        <span className="font-medium hover:text-slate-600 transition-colors cursor-pointer">
          {breadcrumb.parent}
        </span>
        <span className="material-symbols-outlined text-xs">chevron_right</span>
        <span className="text-[#0B1C30] font-bold text-sm tracking-tight">
          {breadcrumb.child}
        </span>
      </div>
                ...
                <div className="max-h-80 overflow-y-auto divide-y divide-slate-100">
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((notif) => (
                      <div
                        key={notif.id}
                        onClick={() => !notif.read && notif.id && handleMarkRead(notif.id)}
                        className={`p-3.5 transition-colors hover:bg-slate-50 cursor-pointer ${
