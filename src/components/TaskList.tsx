...
import { AppScreen, GeneratedImageResult, GenerationTask, ProductAsset, ResultTemplate, TaskDetailNavigation, TaskStatus, VideoTaskEntryContext, IMAGE_GENERATION_TYPE_LABELS } from '../types';
...
interface TaskListProps {
  ...
  detailNavigation?: TaskDetailNavigation | null;
  onDetailNavigationHandled?: () => void;
...
export const TaskList: React.FC<TaskListProps> = ({ tasks, products, onUpdateTask, setScreen, onCreateVideo, detailNavigation, onDetailNavigationHandled, canPublishTemplate, canManageTemplate, publishedTemplateTaskIds, onPublishTemplate, getTemplateStatusForTask, onToggleTemplateStatus }) => {
  ...
  useEffect(() => {
    ...
    if (kind !== detailNavigation.mediaType) {
      setKind(detailNavigation.mediaType);
      setStatus('all');
      setQuery('');
      return;
    }
    const group = groups.find((item) => item.tasks.some((task) => task.id === detailNavigation.taskId)
      || (detailNavigation.groupId !== undefined && item.id === detailNavigation.groupId));
    if (!group) return;
    openDrawer(group, detailNavigation.taskId);
    onDetailNavigationHandled?.();
  }, [detailNavigation, groups, kind, onDetailNavigationHandled]);
  ...
  return <div className="space-y-5">
    ...
    {selectedGroup && <TaskDetailsDrawer tasks={selectedGroup.tasks} products={products} initialTaskId={selectedTaskId} onClose={() => setSelectedGroup(null)} onUpdateTask={onUpdateTask} onCreateVideo={onCreateVideo} canPublishTemplate={canPublishTemplate} canManageTemplate={canManageTemplate} publishedTemplateTaskIds={publishedTemplateTaskIds} onPublishTemplate={onPublishTemplate} getTemplateStatusForTask={getTemplateStatusForTask} onToggleTemplateStatus={onToggleTemplateStatus} />}
