...
export function ModelProfileCreator({
  ...
  const [mode, setMode] = useState<ModelGenerationMode>('text');
  ...
  const chooseMode = (next: ModelGenerationMode) => {
    if (working || taskId) return;
    setMode(next);
    setPreflight(undefined);
    setCandidates([]);
    setSelectedResultIds(new Set());
  ...
  const save = async () => {
    if (selectedResultIds.size === 0 || !profileName.trim()) return;
    setWorking(true);
    try {
      ...
      toast.success(`已保存 ${selectedResultIds.size} 个模特到资源库`);
      onPublished();
      onClose();
    } catch (error) {
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm">
      <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-2xl">
        <header className="flex items-start justify-between border-b border-slate-200 px-6 py-4">
          ...
          <button
            onClick={onClose}
            className="grid h-8 w-8 place-items-center rounded text-slate-400 hover:bg-slate-100"
            aria-label="关闭"
          >
              ...
              {selectedResultIds.size > 0 && (
                <div className="mt-4 border-t border-slate-100 pt-4">
                  <p className="mb-2 text-[11px] text-slate-500">
                    已选择 {selectedResultIds.size} 张；多选时将分别创建模特档案。
                  </p>
                  <input
                    value={profileName}
